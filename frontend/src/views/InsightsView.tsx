...
export const InsightsView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-surface-container-high p-5 rounded border border-outline-variant/30 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="inline-flex bg-surface-container-lowest p-1 rounded border border-outline-variant/30 shadow-xs font-mono text-xs">
          ...
          <button
            ...
            disabled={exporting}
            className="px-3 py-1.5 bg-surface-container-lowest hover:bg-surface-container rounded border border-outline-variant/40 text-xs font-mono font-bold text-on-surface flex items-center gap-1 transition-colors"
          >
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <section className="bg-surface-container-low rounded p-5 shadow-xs border border-outline-variant/30 space-y-3">
            <div className="flex items-center justify-between border-b border-outline-variant/20 pb-2">
              ...
              <span className="px-2 py-0.5 rounded bg-primary-fixed text-on-primary-fixed font-mono text-[10px] font-bold tracking-wider uppercase">
                [HIGH CONFIDENCE]
              </span>
            </div>
            ...
            <div className="bg-surface-container-lowest p-3.5 rounded border border-outline-variant/20 flex items-start gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* 1. Ticket Volume */}
            <div className="bg-surface-container-low p-4 rounded border border-outline-variant/30 shadow-xs flex flex-col justify-between space-y-2">
              ...
              <div className="font-mono text-[10px] text-on-surface-variant border-t border-outline-variant/20 pt-1">
                Active Cycle Count: {insights.total_cases - insights.resolved_cases}
              </div>
            </div>
            ...
            <div className="bg-surface-container-low p-4 rounded border border-outline-variant/30 shadow-xs flex flex-col justify-between space-y-2">
              <div className="flex items-center justify-between text-[11px] font-mono text-on-surface-variant uppercase font-bold">
                ...
                {insights.sla_compliance_rate_percent >= 90 ? (
                  <span className="px-1.5 py-0.2 rounded bg-primary-fixed text-on-primary-fixed font-mono text-[9px] font-bold">
                    OPTIMAL
                  </span>
                ) : (
                  <span className="px-1.5 py-0.2 rounded bg-error text-on-error font-mono text-[9px] font-bold">
                    BREACH
                  </span>
                )}
              </div>
            ...
            <div className="bg-surface-container-low p-4 rounded border border-outline-variant/30 shadow-xs flex flex-col justify-between space-y-2">
              ...
              <div>
                ...
                <div className="font-sans text-xs text-on-surface-variant mt-0.5">
                  Resolution compliance: {insights.resolve_compliance_rate_percent}%
                </div>
              </div>
              <div className="font-mono text-[10px] text-on-surface-variant border-t border-outline-variant/20 pt-1">
                First-response compliance: {insights.response_compliance_rate_percent}%
              </div>
            </div>
            ...
            <div className="bg-surface-container-low p-4 rounded border border-outline-variant/30 shadow-xs flex flex-col justify-between space-y-2">
              ...
              <div className="font-mono text-[10px] text-primary border-t border-outline-variant/20 pt-1">
                Reopened count: {insights.reopened_cases}
              </div>
            </div>
          </div>

          {/* Team Performance Table */}
          <div className="bg-surface-container-low rounded p-5 border border-outline-variant/30 shadow-xs space-y-3">
            <h2 className="font-headline text-base font-bold text-on-surface">
              ...
              <table className="w-full text-left font-mono text-xs">
                <thead>
                  <tr className="border-b border-outline-variant/30 text-on-surface-variant text-[11px]">
                    ...
                    <th className="py-2 px-3">Assigned Cases</th>
                    <th className="py-2 px-3">Resolved Cases</th>
                ...
                <tbody>
                  {insights.team_metrics && insights.team_metrics.length > 0 ? (
                    insights.team_metrics.map((tm) => (
                      <tr key={tm.team_id} className="border-b border-outline-variant/20 hover:bg-surface-container-high/40">
                        <td className="py-2.5 px-3 font-bold text-on-surface">{tm.team_name}</td>
                        <td className="py-2.5 px-3">{tm.assigned_count}</td>
                        <td className="py-2.5 px-3 text-primary font-bold">{tm.resolved_count}</td>
                        <td className="py-2.5 px-3">
                          {tm.breach_count > 0 ? (
                            <span className="text-error font-bold">{tm.breach_count}</span>
                          ) : (
